# Linux Handbook 🐧

A practical and structured **Linux Handbook** covering essential Linux commands, system administration, process management, services, security, networking, troubleshooting, and DevOps-related Linux concepts.

This repository is designed for **beginners, system administrators, DevOps engineers, Cloud engineers, and anyone preparing for Linux interviews**.

---

## 15-Day Linux Learning Series

| Day | Topic                                   | Repository                                                     |
| --: | --------------------------------------- | -------------------------------------------------------------- |
|  01 | **Linux Introduction & Virtualization** | [View Topic](./01-Linux%20Introduction%20&%20Virtualization/)  |
|  02 | **Linux Filesystem & File Operations**  | [View Topic](./02-Linux%20Filesystem%20&%20File%20Operations/) |
|  03 | **Linux Text Processing & Search**      | [View Topic](./03-Linux%20Text%20Processing%20&%20Search/)     |
|  04 | **Linux Redirection & Productivity**    | [View Topic](./04-Linux%20Redirection%20&%20Productivity/)     |
|  05 | **Linux Boot & System Management**      | [View Topic](./05-Linux%20Boot%20&%20System%20Management/)     |
|  06 | **Processes & Process Management**      | [View Topic](./06-Processes%20&%20Process%20Management/)       |
|  07 | **Disk, Storage & Permissions**         | [View Topic](./07-Disk,%20Storage%20&%20Permissions/)          |

---

### Repository

**Linux Handbook:**
https://github.com/Rajeshsnaik/linux-handbook

> **15 Days → Linux Fundamentals → System Administration → Shell Scripting → Interview Preparation**

---

## Follow More

Follow my **Linux & DevOps learning journey** as I continue covering Linux, Networking, Git, Docker, Kubernetes, AWS, Terraform, CI/CD, and other DevOps technologies.

⭐ **Star the repository** if you find these notes useful.

**GitHub Profile:** [GitHub](https://github.com/Rajeshsnaik)

**LinkedIn:** [LinkedIn](https://www.linkedin.com/in/rajeshnaik2002/)
